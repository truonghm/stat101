...
Cho $A$ là một tập hợp hữu hạn có $n$ phần tử, $n\geqslant1$. Hoán vị của $A$ là một cách sắp xếp tự $n$ phần tử của $A$. Nói cách khác, một hoán vị của $A$ là ánh xạ 1-1 (one-to-one mapping) của $A$ lên chính nó.
...
Để miêu tả cụ thể một hoán vị của tập hợp $A$, trước tiên ta liệt kê các phần tử của $A$, cũng như vị trí mà mỗi phần tử được "chuyển đến" (qua ánh xạ). Ví dụ, nếu $A={a,b,c}$ thì một hoán vị $\sigma$ có thể xảy ra là:

$$a={a\ \ b\ \ c\choose b\ \ c\ \ a}$$
Thông qua phép hoán vị $\sigma$, phần tử $a$ chuyển đến $b$, $b$ chuyển đến $c$ và $c$ chuyển đến $a$. Điều kiện của ánh xạ là 1-1 (one-to-one/đơn ánh), nghĩa là không tồn tại hai phần tử của $A$ được chuyển đến cùng một phần tử khác của $A$ thông qua ánh xạ.
