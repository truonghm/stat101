...
Sử dụng R, ta có thể kiểm chứng công thức Stirling như sau:
...
# # for (i in 1:3){
# n=10
# df <- data.frame(permutations(n,n))
# df2 <- data.frame("id"=seq.int(nrow(df)))
# 
# for (j in 1:n){
#   df2<-cbind(df2, df[[paste0('X',j)]] == j)
# }
# 
# names(df2) <- c("id",c(sprintf("X", 1:10)))
# fixed <- select(df2,-id)*1 %>%
#   mutate(sum1 = rowSums(.[1:10]))
